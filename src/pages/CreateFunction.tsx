...
import { Upload, FileCode, Container, FileUp, FolderUp, X, AlertTriangle, CheckCircle2, Maximize2, Minimize2, Settings } from "lucide-react";
...
export default function CreateFunction() {
  ...
  const isEditorFullscreen = useRef(false); // Use ref for internal state, not for re-render
  const [fullscreenState, setFullscreenState] = useState(false); // State to force re-render for fullscreen class
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Exclude file objects as they can't be serialized to storage
  ...
  useEffect(() => {
    // Save to storage excluding binary files
  ...
  const isFormValid = () => {
    ...
    // Docker check would go here
    return true;
  };

  const handleCreate = (status: "active" | "inactive") => {
    const finalData = { ...formData, status };
    console.log(`Creating function (${status}): `, finalData);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => handleCreate('inactive')} disabled={!isFormValid()}>
              Save Draft
            </Button>
            <Button onClick={() => handleCreate('active')} disabled={!isFormValid()}>
              Deploy Function
            </Button>
          </div>
        }
